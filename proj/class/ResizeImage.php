...
function ResizeImage($img, $dst_img='', $corner, $wid, $hei,$c, $corner_radius, $angle)
   {
       $this->srcimg = $img;
	   $this->corner = $corner;
       ...
       //目标图象地址
       $this->dstimg = $dst_img;
       !!empty($this->dstimg) && header('Content-Type: image/png');
//       $this -> dst_img();
       //--
   ...
   function initSize(){
   		!$this->resize_width && $this->resize_width = $this->width;
   		!$this->resize_height && $this->resize_height = $this->height;
   		
		$_hb = $this->resize_height/$this->height;
		$_wb = $this->resize_width/$this->width;
		if ($_hb == min($_hb,$_wb)){
			$this->resize_width = $this->width*$_hb;
		}else $this->resize_height = $_wb*$this->height;
   }
   function newimg()
   {
       //改变后的图象的比例
       ...
       //裁图
       {
           ...
           //高度优先
           {
               ...
               !!empty($this->dstimg) ? imagepng ($tmp) : imagepng ($tmp,$this->dstimg);
           }
           if($ratio<$resize_ratio)
           //宽度优先
           {
       ...
       //不裁图
       {
           if($ratio>=$resize_ratio)
           {
               ...
               !!empty($this->dstimg) ? ImageJpeg ($newimg) : ImageJpeg ($newimg,$this->dstimg);
           }
           if($ratio<$resize_ratio)
           {
