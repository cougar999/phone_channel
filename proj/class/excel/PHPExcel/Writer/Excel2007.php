...
class PHPExcel_Writer_Excel2007 implements PHPExcel_Writer_IWriter
{
	...
	private $_template_zip_object = null;
    /**
    ...
    public function __construct(PHPExcel $pPHPExcel = null)
    {
		...
		//wilson add,2010-09-09
		$this->_template_zip_object = $pPHPExcel->getZipObject();
    }
	...
	public function save_with_template()
	{
		//有读取的zip object.
		//此模式当前不支持增加sheet或者修改sheet名称。可以修改数据的仅限标记data的sheet.
		  $zip_object = $this->_template_zip_object;
			$this->_stringTable = array();
			$sheet_list = $this->_spreadSheet->getAllSheets();
			...
			foreach($sheet_list as $id => $sheet_obj)
			{
				$sheet_name = $sheet_obj->getTitle();
				if (strlen($sheet_name)>=5 && substr($sheet_name,0,5)=='Sheet')
				{
					$str1 = 'xl/worksheets/'.strtolower($sheet_name).'.xml';
					if ($zip_object->getFromName($str1))
					{
						$zip_object->deleteName($str1);
						$zip_object->addFromString($str1, $this->getWriterPart('Worksheet')->writeWorksheet($this->_spreadSheet->getSheet($id), $this->_stringTable));
					}
				}
			}
/*
			for ($i = 0; $i < $this->_spreadSheet->getSheetCount(); ++$i) 
			{
				$str1 = 'xl/worksheets/sheet' . ($i + 1) . '.xml';
				if ($zip_object->getFromName($str1))
				{
					$zip_object->deleteName($str1);
					$zip_object->addFromString($str1, $this->getWriterPart('Worksheet')->writeWorksheet($this->_spreadSheet->getSheet($i), $this->_stringTable));
				}
			}
*/
			// Add string table to ZIP file
			$str1 = 'xl/sharedStrings.xml';
			$zip_object->deleteName($str1);
			$zip_object->addFromString($str1, $this->getWriterPart('StringTable')->writeStringTable($this->_stringTable));

			return $zip_object->close();
	}
