...
//				echo 'Adjusted Formula is '.$value.'<br />';
	...
	public function load_for_template($pFilename,$template_out_filename)
	{
		if (strlen($template_out_filename)<=0)
		{
			throw new Exception("Template out filename is empty!");
		}
		
		if (copy($pFilename, $template_out_filename) === false) {
			throw new Exception("Could not copy temporary zip file $pFilename to $template_out_filename.");
		}
		return $this->load($template_out_filename);
	}
