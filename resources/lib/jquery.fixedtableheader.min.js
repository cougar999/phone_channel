...
jQuery.fn.fixedtableheader = function(options) {
    ...
    this.each(function(i) {
        ...
        if ($tblhfixed.find(headerelement).length > 0) {
            ...
            var $clonedTable = $tbl.clone(true).empty();
            var tblwidth = GetTblWidth($tbl);
			$.each(settings.addclass,function(k,v){
				$clonedTable.addClass(v);
			})
            $clonedTable.attr("id", "fixedtableheader" + i).css({
                "position": "fixed",
                "top": "0",
                "left": $tbl.offset().left
            }).append($tblhfixed.clone(true)).width(tblwidth).hide().appendTo($("body"));
