...
function paintFlashChart(data, fcId, options) {
    var defaults = {
        swfUrl : '/resources/stats/anychart/AnyChart.swf', // flash 地址
        width : '100%', // flash 宽度
        height : 200, // flash 高度
...
function showLoadingDiv(id) {
    ...
    $(id_class).each(function() {
        ...
        var $_tmp_div = jDiv.clone();
        $(this).parent().append(jDiv);
        $(this).parent().append($_tmp_div);
    });
}
...
function orderAjax(url, order, ascDesc, className, wrapId) {
    ...
    // 为表格列加上 click 事件
    $('#' + wrapId+',#dataTheadTr_fex').find('td, th').each(function() {
        if($(this).hasClass(className)) {
            ...
            $(orderObj).bind('click', function() {
                ...
                showData({sortname:currentOrder,sortorder:'desc' == ascDesc ? 'asc' : 'desc'});
                return false;
                doAjax({
                    type : 'GET',
                    url : ajaxUrl + '&ajax=1'
...
function showWindows(content, title, callback, btnType, extra) {
	...
	if (btnType != -1 && btnText1) {
		...
		if (btnText2) {
			btnHtml += '&nbsp;&nbsp;<button class="pn" id="btn_1" onclick0="$(\'#fwin_dialog_close\').click();"><span>' + btnText2 + '</span></button>';
		}
		btnHtml += '</div>';
	} else {
		btnHtml = '';
