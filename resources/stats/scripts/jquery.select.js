...
$.fn.extend({
	sSelect: function() {
		...
		$('#'+selectId).append('<div class="dropselectbox"><h6></h6><ul></ul></div>');
		$('#'+selectId+' h6').empty().append($('#'+selectId+' option:selected').text());
		var selectWidth=$('#'+selectId+' select').width();
		if($.browser.safari){selectWidth = selectWidth+15}
		$('#'+selectId+' h6').css({width:selectWidth});
		var selectUlwidth = selectWidth + parseInt($('#'+selectId+' h6').css("padding-left")) + parseInt($('#'+selectId+' h6').css("padding-right"));
		$('#'+selectId+' ul').css({width:selectUlwidth+8+'px'});
		$('#'+selectId+' select').hide();
		$('#'+selectId+' div').hover(function(){
			$('#'+selectId+' h6').addClass("over");
		},function(){
			$('#'+selectId+' h6').removeClass("over");
		});
		$('#'+selectId)
		.bind("focus",function(){
		...
		.bind("click",function(e){
			if($('#'+selectId+' ul').css("display") == 'block'){
				...
				windowspace < ulspace && windowspace2 > 0?$('#'+selectId+' ul').css({top:-ulspace}):$('#'+selectId+' ul').css({top:$('#'+selectId+' h6').outerHeight(true)});
				$(window).scroll(function(){
					windowspace = ($(window).scrollTop() + document.documentElement.clientHeight) - $('#'+selectId).offset().top;
					windowspace < ulspace?$('#'+selectId+' ul').css({top:-ulspace}):$('#'+selectId+' ul').css({top:$('#'+selectId+' h6').outerHeight(true)});
				});
	...
	clearSelectMenu:function(selectId,selectZindex){
		if('undefined' != typeof(selectId)){
			$('#'+selectId+' ul').empty().hide();
			$('#'+selectId+' h6').removeClass("over").removeClass("current");
			if(selectZindex) {
	...
	setSelectValue:function(sID){
		...
		$('#'+sID+' ul').html(content);
		$('#'+sID+' h6').html($('#'+sID+' option:selected').text());
